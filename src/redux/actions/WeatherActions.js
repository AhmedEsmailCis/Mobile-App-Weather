...
export const getDataOfWeather = ({ lat, lon }) => {
  return (dispatch) => {
    ...
    dispatch({ type: "updateLocation", lat, lon });
    loadWeatherData(dispatch, lat, lon);
  };
};
const loadWeatherData = async (dispatch, lat, lon) => {
  const url1 = `${oneCallApi}?lat=${lat.toString()}&lon=${lon.toString()}&units=${units}&appid=${apiId}`;
  const url2 = `${onlyCurrentApi}?lat=${lat.toString()}&lon=${lon.toString()}&units=${units}&appid=${apiId}`;
...
const getCityCoord = async (dispatch, cityName) => {
  const url = `${onlyCurrentApi}?q=${cityName.toLowerCase()}&units=${units}&appid=${apiId}`;
